...
void CardLayout::setGeometry(const QRect &r)
{
    ...
    int numberOfPlayedCards = playWindow->getGame()->getPlayedCardsHistory().size();
    showCardsFaceUp = (contract.getIsSet() && player->getPosition() == dummyPosition) && numberOfPlayedCards >= 1;
    if ((playerPosition==0)and (PositionNorth )) showCardsFaceUp =true;
    if ((playerPosition==1)and (PositionEast )) showCardsFaceUp =true;
    if ((playerPosition==2)and (PositionSouth )) showCardsFaceUp =true;
    if ((playerPosition==3)and (PositionWest )) showCardsFaceUp =true;


   // bool showCardsFaceUp =true;
    int i = 0;
